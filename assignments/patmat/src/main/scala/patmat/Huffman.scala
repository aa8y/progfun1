...
object Huffman {
  ...
  def convert(tree: CodeTree): CodeTable = {
    def loop(subTree: CodeTree, bitAcc: List[Bit], partTable: CodeTable): CodeTable = subTree match {
      case Fork(left, right, char, _) =>
        loop(left, bitAcc :+ 0, partTable) ::: loop(right, bitAcc :+ 1, partTable)
      case Leaf(char, _) => partTable :+ (char, bitAcc)
    }
    loop(tree, Nil, Nil)
  }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    text.foldLeft(List[Bit]()) { (partEnc, char) => partEnc ::: codeTable(char) }
  }
}
